 ...
 * @usage low
 *
 * @param {...any} params The parameters for the hook. Can be (initialValue?: number, options?: UseCounterOptions) or (options: UseCounterOptions & { initialValue?: number }, initialValue?: never)
...
export const useCounter = ((...params: any[]) => {
  ...
  const inc = (value: number = 1) => {
    setValue(prevValue => {
      if (typeof max === 'number' && prevValue === max)
return prevValue
  ...
  const dec = (value: number = 1) => {
    setValue(prevValue => {
      if (typeof min === 'number' && prevValue === min)
return prevValue
